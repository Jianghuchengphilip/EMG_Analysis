...
asc_file_path = './data/asc/1.asc'
emg_data,channels_name = Read_Asc_File(asc_file_path)
dc_removed_data = remove_dc_offset(emg_data)
filtered_data = Butter_Bandpass_Preprocessing(dc_removed_data,1000)
# rectified_data = full_wave_rectification(filtered_data)
envelope_data = rms_envelope(filtered_data)
...
Plot_RMS_MAV_Graph(rms_values,mav_values,channels_name)
# Plot_PSD_Graph(psd_values)
# NMF_W,NMF_H = NMF_Muscle(normalized_data,5)
# print(NMF_W.shape,NMF_H.shape)
# Plot_NMF_Graph(NMF_W,NMF_H,3,channel_names,"NMF")
