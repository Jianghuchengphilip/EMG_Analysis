...
#分割步态周期
def Gait_Cycle_Segmentation(filtered_emg_data,event_indices):
    cycles = []
    for i in range(len(event_indices) - 1):
        start = event_indices[i]
        end = event_indices[i + 1]
        cycles.append(filtered_emg_data[:, start:end])
    return cycles
...
def Compute_Psd_Per_Cycle(cycles, sampling_rate):
    ...
    return cycle_psds
#肌肉协同分析
def NMF_Muscle(filtered_emg_data,n_components):
    # 对数据进行NMF分解
    ...
    return W,H
#左右肌群对比
def Left_Right_Muscle_Comparison(filtered_emg_data):
...
def Muscle_CCI(filtered_emg_data):
